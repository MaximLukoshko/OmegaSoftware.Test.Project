================================================================================
    БИБЛИОТЕКА MICROSOFT FOUNDATION CLASS. Обзор проекта OmegaSoftware
===============================================================================

Это приложение OmegaSoftware автоматически создано с помощью мастера приложений. Это приложение демонстрирует основные принципы использования классов Microsoft Foundation; оно также может использоваться в качестве основы для написания собственных приложений.

В этом файле представлена сводка содержимого всех файлов, входящих в состав приложения OmegaSoftware.

OmegaSoftware.vcxproj
...
OmegaSoftware.h
    ...
    Он содержит прочие заголовки, относящиеся к проекту (включая Resource.h), а также объявляет класс приложения COmegaSoftwareApp.

OmegaSoftware.cpp
    Это основной исходный файл приложения, в котором содержится класс приложения COmegaSoftwareApp.
...
res\OmegaSoftware.ico
    Файл значка, используемый в качестве значка приложения. Этот значок включен с помощью основного файла ресурсов OmegaSoftware.rc.

res\OmegaSoftware.rc2
...
res\Toolbar.bmp
    ...
    Начальная панель инструментов и строка состояния, созданные в классе CMainFrame. Измените этот точечный рисунок панели инструментов в редакторе ресурсов и обновите массив IDR_MAINFRAME TOOLBAR в файле OmegaSoftware.rc, чтобы добавить кнопки панели инструментов.
/////////////////////////////////////////////////////////////////////////////

Мастер приложений создает один тип документов и одно представление:

OmegaSoftwareDoc.h, OmegaSoftwareDoc.cpp — документ
    Эти файлы содержат класс COmegaSoftwareDoc. Измените эти файлы, чтобы добавить особые данные документа и реализовать сохранение и загрузку файлов (посредством COmegaSoftwareDoc::Serialize).

OmegaSoftwareView.h, OmegaSoftwareView.cpp — представление документа
    Эти файлы содержат класс COmegaSoftwareView.
    Объекты COmegaSoftwareView используются для просмотра объектов COmegaSoftwareDoc.
...
StdAfx.h, StdAfx.cpp
    Эти файлы используются для построения файла предкомпилированного заголовка (PCH) с именем OmegaSoftware.pch и файла предкомпилированных типов с именем StdAfx.obj.
